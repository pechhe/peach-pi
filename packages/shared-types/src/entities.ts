...
export interface NoticePayload {
  ...
  /** Optional action button for the toast. Currently only `utility-model`,
   *  which navigates to Settings → utility model so the user can pick a
   *  different model or add credits. */
  action?: { label: string; target: "utility-model" };
}
