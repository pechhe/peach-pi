...
/** Settings-section targets a notice action can navigate to. */
type NoticeActionTarget = "utility-model";

/** Handles `action.target` from notices by navigating in the renderer. Set
 *  from `App.svelte` during init (`extensionUi.setSettingsNav(fn)`) to avoid
 *  a circular import (App owns navigation but the store is wired to IPC).
 *  When unset, action buttons are suppressed and the notice shows text only. */
let settingsNav: ((query?: string) => void) | null = null;

function actionForTarget(target: NoticeActionTarget, label: string): ToastAction | undefined {
...
interface Toast extends Omit<NoticePayload, "action"> {
  id: number;
  action?: ToastAction;
...
class ExtensionUiStore {
  ...
  setSettingsNav(fn: (query?: string) => void): void {
    settingsNav = fn;
  }
  ...
  pushToast(notice: NoticePayload): void {
    const toast: Toast = {
      ...
      action: notice.action
        ? actionForTarget(notice.action.target, notice.action.label)
        : undefined,
    };
