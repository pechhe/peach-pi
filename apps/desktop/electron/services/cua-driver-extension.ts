...
const EXTENSION_SOURCE = [
  ...
  "    description:",
  "      \"List macOS apps — both currently running and installed — with per-app",
  "      \" state (running, active) and, when running, the pid + window ids. Use this",
  "      \" to find a bundle_id before cua_driver_launch_app, or a pid/window_id",
  "      \" for cua_driver_get_window_state / click.\",",
  ...
  "    description:",
  "      \"Launch a macOS app hidden (background, no foreground/focus steal).",
  "      \" Returns the pid and the app's window ids. Use cua_driver_get_window_state",
  "      \" next to read the window's AX tree + element indexes.\",",
  "    parameters: Type.Object({",
  ...
  "    description:",
  "      \"Walk a running app's accessibility tree and return it as Markdown,",
  "      \" tagging every actionable element with [element_index N]. Always call",
  "      \" this BEFORE click/type_text so your element_index is fresh, and call",
  "      \" it again after to verify the result. Set capture_mode to \\\"vision\\\" to",
  "      \" also capture a screenshot PNG (path returned) you can read with",
  "      \" analyze_image when element labels are missing or ambiguous.\",",
  "    parameters: Type.Object({",
  ...
  "    description:",
  "      \"Left-click an element in a native macOS app. Two modes: (1) element_index",
  "      \" + window_id — pure AX RPC, works on backgrounded/hidden windows, no",
  "      \" cursor move or focus steal. Requires a prior cua_driver_get_window_state",
  "      \" on this (pid, window_id) this turn. (2) x, y in window-local screenshot",
  "      \" pixels (top-left origin of the PNG get_window_state returns) — synthesized",
  "      \" mouse events, AX never consulted. Prefer element_index when AX exposes",
  "      \" the target; fall back to pixels for things AX can't see.\",",
  "    parameters: Type.Object({",
  ...
  "    description:",
  "      \"Insert text at the target process's cursor. Optionally focus an",
  "      \" element_index first (needs a prior cua_driver_get_window_state +",
  "      \" window_id). Background-safe — no focus steal.\",",
  "    parameters: Type.Object({",
  "      pid: Type.Integer({ description: \"Target process id.\" }),",
  ...
  "    description:",
  "      \"Capture a PNG screenshot of a window and return its path. Pass the",
  "      \" path to analyze_image to read what's on screen — use this when the",
  "      \" AX tree from get_window_state is thin or ambiguous (Electron apps,",
  "      \" custom-drawn UI). Prefer get_window_state with capture_mode=vision to",
  "      \" inspect + screenshot in one call.\",",
  "    parameters: Type.Object({",
